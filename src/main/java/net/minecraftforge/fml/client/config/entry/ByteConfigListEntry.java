 ...
 * @author Cadiboo
 */
public class ByteConfigListEntry extends ConfigListEntry {

	private final ByteConfigValueElement configValueElement;
	private final TextFieldWidget textFieldWidget;

	public ByteConfigListEntry(final ByteConfigValueElement configValueElement, final ConfigScreen configScreen, final ConfigEntryListWidget configEntryListScreen) {
		...
		this.textFieldWidget.func_212954_a(s -> {
			if (!this.isValidValue())
				return;
			this.configValueElement.set(Byte.valueOf(s));
		});
	}
	...
	public boolean isValidValue() {
		try {
			byte o = Byte.parseByte(this.textFieldWidget.getText());
