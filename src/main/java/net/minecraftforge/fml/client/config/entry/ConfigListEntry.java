...
public abstract class ConfigListEntry<T> extends AbstractOptionList.Entry<ConfigListEntry<?>> {
	...
	protected void makeTooltip(final ConfigScreen owningScreen) {
		...
		final EntryConfigValue<T> configValue = getEntryConfigValue();
		if (configValue != null) {
...
//				Collections.addAll(toolTip, (TextFormatting.AQUA + I18n.format("fml.configgui.tooltip.default", configElement.getDefault())).split("\n"));
			...
			if (configValue.requiresMcRestart() || owningScreen.doAllRequireMcRestart())
	...
	protected abstract EntryConfigValue<T> getEntryConfigValue();
