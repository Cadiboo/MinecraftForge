...
public class NumberSlider<T extends Number & Comparable<? super T>> extends GuiSlider {
	...
	public NumberSlider(final IConfigListEntryWidget.Callback<T> callback, final Range<T> range, final Function<String, T> parser) {
		this("Number Slider", callback, range, parser);
	}

	public NumberSlider(final String message, final IConfigListEntryWidget.Callback<T> callback, final Range<T> range, final Function<String, T> parser) {
