 ...
 * @author Cadiboo
 */
public class DoubleConfigListEntry extends ConfigListEntry {

	private final DoubleConfigValueElement configValueElement;
	private final TextFieldWidget textFieldWidget;

	public DoubleConfigListEntry(final DoubleConfigValueElement configValueElement, final ConfigScreen configScreen, final ConfigEntryListWidget configEntryListScreen) {
		...
		this.textFieldWidget.func_212954_a(s -> {
			if (!this.isValidValue())
				return;
			this.configValueElement.set(Double.valueOf(s));
		});
	}
	...
	public boolean isValidValue() {
		try {
			double o = Double.parseDouble(this.textFieldWidget.getText());
