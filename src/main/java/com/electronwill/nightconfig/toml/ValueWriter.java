		...
		if (value instanceof Config) {
			...
			if (!list.isEmpty() && list.get(0) instanceof Config) {// Array of tables
				for (Iterator<?> iterator = list.iterator(); iterator.hasNext(); ) {
					...
					// TODO: Fixes a bug in NightConfig - Not writing the separator means that the tables can't be parsed. See "https://github.com/TheElectronWill/night-config/pull/75"
					if (iterator.hasNext())
